...
# 创建卫星星系
def Creat_satellite(numOrbitPlanes=6, numSatsPerPlane=5, hight=750, Inclination=53, name=''):
...
def Creating_All_Access():
    ...
    for each_sat in tqdm(sat_list):
        ...
        # 发射机与接收机相连
        # 与后面的卫星的接收机相连
        for eacha_sat in tqdm(sat_list):
            now_sat_namea = eacha_sat.InstanceName
            now_plane_numa = int(now_sat_name.split('_')[0][3:])
            now_sat_numa = int(now_sat_name.split('_')[1])
            now_sat_transmitter = each_sat.Children.GetElements(STKObjects.eTransmitter)[0]  # 找到该卫星的发射机
            Set_Transmitter_Parameter(now_sat_transmitter)  # , EIRP=20)
            if now_sat_name != now_sat_namea:
                access = now_sat_transmitter.GetAccessToObject( Get_sat_receiver(sat_dic['KPA' + str(now_plane_numa) + '_' + str((now_sat_numa + 1) % 5)]))
                access.ComputeAccess()
...
# 如果不是读取当前场景，即首次创建场景
if not Read_Scenario:
    Creat_satellite(numOrbitPlanes=6, numSatsPerPlane=5, hight=750, Inclination=53, name='KPB')# Starlink
    Creat_satellite(numOrbitPlanes=6, numSatsPerPlane=5, hight=450, Inclination=53, name='KPA')
    # Kuiper
