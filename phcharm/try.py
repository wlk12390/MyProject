...
def Creating_All_Access():
    ...
    # 计算某个卫星与其他卫星的链路质量，并生成报告
        if t==0:
                    ...
                    for k in range(0, 2):
                       for j in range(0,3):
                         ...
                         for i in range(0, accessIntervals.Count):
                             durationnumxy = accessIntervals.Count
                             timesi = accessIntervals.GetInterval(i)
                             #print(timesi)
                             sxyi = list(timesi)
                             a1xyi = re.sub("\D", "", sxyi[0])  # 正则表达式
                             b1xyi = re.sub("\D", "", sxyi[1])
                             #return a1xyi,b1xyi,durationnumxy
                         for i in range(0, accessIntervals.Count):
                             if Temp >= int(a1xyi) and Temp <= int(b1xyi):
                                 link_t[x, y] = 1
                                 break
            ...
            for x in range(0, 12):
                for y in range(0, 12):
                    for i in range(0, durationnumxy):
                        a1xyi = re.sub("\D", "", sxyi[0])  # 正则表达式
                        b1xyi = re.sub("\D", "", sxyi[1])
                        # print(a1, b1)
                        if Temp >= int(a1xyi) and Temp <= int(b1xyi):
                            link_t[x, y] = 1
                            break
                    #print(a1xyi,b1xyi,Temp,durationnumxy)
            #print('第', t, '个链接矩阵')
            #print(link_t)
    '''
